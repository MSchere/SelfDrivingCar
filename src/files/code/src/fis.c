...
/* define a standard memory access function with err checking */
void *fisCalloc(int num_of_x, int size_of_x) {
    ...
    if (num_of_x == 0)
        ...
        /* however we still need to check that memory was allocated successfully,
           exclude the case when num_of_x = 0, and if unsuccessful issue an err */
        if (ptr == NULL)
            fisErr("Could not allocate memory in mxCalloc function call.");
    }
    ...
    if (num_of_x == 0)
        ...
        /* however we still need to check that memory was allocated successfully,
           exclude the case when num_of_x = 0, and if unsuccessful issue an err */
        if (ptr == NULL)
            fisErr("Could not allocate memory in calloc function call.");
    }
#endif
...
char **fisCreateMatrix(int row_n, int col_n, int element_size) {
    ...
    if (matrix == NULL)
        fisErr("Calloc err in fisCreateMatrix!");
    for (i = 0; i < row_n; i++) {
...
static DOUBLE fisTriangleMf(DOUBLE x, DOUBLE *params) {
    ...
    if (a > b)
        fisErr("Illegal parameters in fisTriangleMf() --> a > b");
    if (b > c)
        fisErr("Illegal parameters in fisTriangleMf() --> b > c");
...
static DOUBLE fisTrapezoidMf(DOUBLE x, DOUBLE *params) {
    ...
    if (a > b) {
        ...
        fisErr("Illegal parameters in fisTrapezoidMf() --> a > b");
    }
    if (b > c) {
        PRINTF("a = %f, b = %f, c = %f, d = %f\n", a, b, c, d);
        fisErr("Illegal parameters in fisTrapezoidMf() --> b > c");
    }
    if (c > d) {
        PRINTF("a = %f, b = %f, c = %f, d = %f\n", a, b, c, d);
        fisErr("Illegal parameters in fisTrapezoidMf() --> c > d");
    }
...
static DOUBLE fisGaussianMf(DOUBLE x, DOUBLE *params) {
    ...
    if (sigma == 0)
        fisErr("Illegal parameters in fisGaussianMF() --> sigma = 0");
    tmp = (x - c) / sigma;
...
static DOUBLE fisGaussian2Mf(DOUBLE x, DOUBLE *params) {
    ...
    if ((sigma1 == 0) || (sigma2 == 0))
        fisErr("Illegal parameters in fisGaussian2MF() --> sigma1 or sigma2 is zero");
...
static DOUBLE fisGeneralizedBellMf(DOUBLE x, DOUBLE *params) {
    ...
    DOUBLE tmp;
    if (a == 0)
        fisErr("Illegal parameters in fisGeneralizedBellMf() --> a = 0");
    tmp = pow((x - c) / a, 2.0);
    if (tmp == 0 && b == 0)
        return (0.5);
    else if (tmp == 0 && b < 0)
...
void fisAssignFunctionPointer(FIS *fis) {
    ...
    if (strcmp(fis->andMethod, "prod") == 0)
        ...
        fisErr("Legal andMethod: min, prod");
#endif
    }
    ...
    if (strcmp(fis->orMethod, "probor") == 0)
        ...
        fisErr("Legal orMethod: max, probor");
#endif
    }
    ...
    if (strcmp(fis->impMethod, "prod") == 0)
        ...
        fisErr("Legal impMethod: min, prod");
#endif
    }
    ...
    if (strcmp(fis->aggMethod, "max") == 0)
        ...
        fisErr("Legal aggMethod: max, probor, sum");
#endif
    }
    ...
    if (strcmp(fis->defuzzMethod, "centroid") == 0)
        ...
        fisErr("Legal defuzzification methods: centroid, bisector, mom, som, lom, wtaver, wtsum");
#endif
    }
}
...
static void fisPrintData(FIS *fis) {
    ...
    if (fis == NULL)
        fisErr("Given fis pointer is NULL, no data to print!");
...
/* Returns a FIS pointer if there is a match; otherwise signals err */
static FIS *fisMatchHandle(FIS *head, int handle) {
    ...
    if (p == NULL) {
        ...
        fisErr("Cannot find an FIS with this handle.");
    }
    return (p);
}
    ...
    if (strcmp(fis->type, "sugeno") == 0) {
        ...
        for (i = 0; i < fis->out_n; i++) {
            ...
            if (strcmp(fis->defuzzMethod, "wtaver") == 0)
                ...
                fisErr("Legal methods: wtaver, wtsum");
            }
        }
...
#endif
{
    ...
    if (getNextLine(buf, fp) == NULL)
        fisErr("getNumber: Incomplete FIS file!");

    tmp = sscanf(buf, " %[^=] = %lf ", string, &num);
    if (tmp != 2) {
        PRINTF("Err format in FIS file when parsing\n");
        PRINTF("\"%s\"\n", buf);
        fisErr("Err in getNumber().");
    }
    /*
...
#endif
{
    ...
    if (getNextLine(buf, fp) == NULL)
        fisErr("getString: Incomplete FIS file!");

    tmp = sscanf(buf, " %[^'] '%[^']' ", string1, string2);
    if (tmp != 2) {
        PRINTF("Err format in FIS file when parsing\n");
        PRINTF("\"%s\"\n", buf);
        fisErr("Err in getString().");
    }
...
#endif
{
    ...
    for (i = 0; i < in_n + out_n; i++) {
        while (1) {
            if (getNextLine(buf, fp) == NULL)
                fisErr("Not enough NumMFs in FIS file!");
...
#endif
{
    ...
    while (1) {
        if (getNextLine(buf, fp) == NULL)
            fisErr("Cannot find NumInputs in FIS file!");
    ...
    while (1) {
        if (getNextLine(buf, fp) == NULL)
            fisErr("Cannot find NumOutputs in FIS file!");
    ...
    while (1) {
        if (getNextLine(buf, fp) == NULL)
            fisErr("Cannot find NumRules in FIS file!");
    ...
    while (1) {
        if (getNextLine(buf, fp) == NULL)
            fisErr("Cannot find FIS Name in FIS file!");
    ...
    while (1) {
        if (getNextLine(buf, fp) == NULL)
            fisErr("Cannot find FIS Type in FIS file!");
...
#endif
{
    ...
    while (1) {
        if (getNextLine(buf, fp) == NULL)
            fisErr("Cannot find [System] in FIS file!");
    ...
    while (1) {
        if (getNextLine(buf, fp) == NULL)
            fisErr("Cannot find the first Name in FIS file!");
    ...
    for (i = 0; i < in_n + out_n; i++) {
        while (1) {
            if (getNextLine(buf, fp) == NULL)
                fisErr("Not enough Name in FIS file!");
            if (sscanf(buf, " Name = '%[^']' ", str1) == 1)
                break;
        }
    ...
    for (i = 0; i < in_n + out_n; i++) {
        while (1) {
            if (getNextLine(buf, fp) == NULL)
                fisErr("Not enough Range in FIS file!");
            if (sscanf(buf, " Range = [ %[^]] ", str1) == 1)
                break;
        }
        if (getArray(str1, fismatrix[now++]) != 2)
            fisErr("Err in parsing I/O ranges.");
    }
    fclose(fp);
    ...
    for (i = 0; i < in_n + out_n; i++) {
        ...
        for (j = 0; j < mf_n; j++) {
            while (1) {
                if (getNextLine(buf, fp) == NULL)
                    fisErr("Not enough MF Labels in FIS file!");
                if (sscanf(buf, " %[^']'%[^']' : '%[^']' , [ %[^]] ",
    ...
    for (i = 0; i < in_n + out_n; i++) {
        ...
        for (j = 0; j < mf_n; j++) {
            while (1) {
                if (getNextLine(buf, fp) == NULL)
                    fisErr("Not enough MF types in FIS file!");
                if (sscanf(buf, " %[^']'%[^']' : '%[^']' , [ %[^]] ",
                        str1, str2, str3, str4) == 4)
                    break;
            }
    ...
    for (i = 0; i < in_n + out_n; i++) {
        ...
        for (j = 0; j < mf_n; j++) {
            while (1) {
                if (getNextLine(buf, fp) == NULL)
                    fisErr("Not enough MF parameters in FIS file!");
                if (sscanf(buf, " %[^']'%[^']' : '%[^']' , [ %[^]] ",
                        str1, str2, str3, str4) == 4) {
                    /*
                    printf("%s\n", buf);
            ...
            if (i < in_n) {
                if (getArray(str4, fismatrix[now]) > MF_PARA_N) {
                     ...
                     */
                    fisErr("Err in parsing input MF parameters.");
                }
            } else {
                if (!strcmp(fisType, "mamdani")) {
                    if (getArray(str4, fismatrix[now]) > MF_PARA_N) {
                        fisErr("Err in parsing output MF parameters.");
                    }
                } else { /* sugeno system */
                    int tmp = getArray(str4, fismatrix[now]);
                    if (!strcmp(str3, "constant")) {
                        if (tmp != 1)
                            fisErr("Zero-order Sugeno system does not has the right number of output MF parameters.");
                        ...
                        if (tmp != in_n + 1)
                            fisErr("First-order Sugeno system does not has the right number of output MF parameters.");
                    } else {
                        fisErr("Unknown output MF type for Sugeno system.");
                    }
                }
            }
            now++;
    ...
    while (1) {
        if (getNextLine(buf, fp) == NULL)
            fisErr("Cannot find [Rules] in FIS file!");
        if (!strcmp(buf, "[Rules]")) /* found it! */
            break;
    }
    for (i = 0; i < rule_n; i++) {
        if (getNextLine(buf, fp) == NULL)
            fisErr("Not enough rule list in FIS file!");
        /* get rid of ",", "(" and ")" */
        ...
        if (getArray(buf, fismatrix[now++]) != in_n + out_n + 2) {
             ...
             */
            fisErr("Err in parsing rule list!");
        }
    }
    fclose(fp);
    ...
    for (i = 0; i < row_n; i++) {
        for (j = 0; j < col_n; j++) {
            if (fscanf(fp, "%lf", &num1) != EOF)
                ...
                fisErr("Not enough data in data file!");
        }
    }
    fclose(fp);
